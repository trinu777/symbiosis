...
/* -------- Gallery: placeholders + lightbox --------
   Si las fotos reales aún no están en /assets/gallery/, se generan
   placeholders identificables para reemplazar después. */
function initGallery(){
  const grid = document.getElementById('galleryGrid');
  const PLACEHOLDER_COUNT = 8;
  const ratios = [1.2, 0.8, 1, 1.4, 0.9, 1.1, 0.75, 1.3]; // alturas variables
  ...
  for(let i = 0; i < PLACEHOLDER_COUNT; i++){
    ...
    const localSrc = `assets/gallery/photo-${String(i + 1).padStart(2,'0')}.jpg`;
    const fallbackSrc = `https://picsum.photos/seed/symbiosis${i}/700/${Math.round(700 * ratios[i % ratios.length])}`;
